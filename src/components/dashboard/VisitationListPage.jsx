...
export default function VisitationListPage({ basePath = "/dashboard/doctor", roleLabel = "Doctor" }) {
  ...
  useEffect(() => {
    ...
    async function fetchVisitations() {
      ...
      try {
        ...
        const response = await fetch(`/api/doctor/visitationDisplay?${params.toString()}`);
        const data = await response.json();

        if (!active) return;
        if (!response.ok) throw new Error(data.message || "Failed to load visitations.");
  ...
  const filteredRows = useMemo(() => {
    return visitations.filter((visit) => {
      const matchesSearch = !search.trim() || [visit.name, visit.patientid, visit.status].join(" ").toLowerCase().includes(search.trim().toLowerCase());
      const matchesStatus = !statusFilter || visit.status === statusFilter;
      const matchesPriority = !priorityFilter || visit.priority === priorityFilter;
      return matchesSearch && matchesStatus && matchesPriority;
    });
  }, [visitations, search, statusFilter, priorityFilter]);
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-5 lg:h-[calc(100dvh-4rem)] lg:overflow-hidden">
      <header className="rd-panel relative flex-none overflow-hidden">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-x-8 gap-y-5 px-6 py-5">
          <div>
            ...
            <h1 className="mt-1.5 text-2xl font-bold tracking-tight text-rd-title">Visitations</h1>
            <p className="mt-1.5 text-sm text-rd-muted">Review active laboratory requests and open each visit in detail.</p>
          </div>
        </div>
      </header>
      ...
      <section className="rd-panel flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="flex flex-none flex-wrap items-center justify-between gap-4 border-b border-rd-hair p-4">
          <div>
            <h2 className="text-lg font-semibold text-rd-title">Laboratory queue</h2>
            <p className="mt-0.5 text-sm text-rd-muted">{loading ? "Loading visitations…" : `${filteredRows.length} records`}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rd-input">
        ...
        <div className="rd-scroll-thin min-h-0 flex-1 overflow-auto">
          {loading ? (
            ...
            <EmptyState title="Could not load visitations" hint="Something went wrong reaching the server. Refresh the page to try again." />
          ) : filteredRows.length === 0 ? (
            <EmptyState title="No visitations found" hint="Try another search or clear filters." />
          ) : (
            <table className="w-full min-w-[760px] border-collapse">
              ...
              <tbody>
                {filteredRows.map((visit) => (
                  <tr key={visit.id || visit.visitid} className="border-b border-rd-hair transition-colors last:border-0 hover:bg-rd-raised">
                    <td className={td}>
                      <div className="flex items-center gap-3">
                        <Avatar name={visit.name} />
                        <div className="min-w-0">
                          <p className="truncate font-medium text-rd-title">{visit.name}</p>
                          <p className="truncate text-xs text-rd-muted">ID #{visit.patientid || visit.id}</p>
                        </div>
                      </div>
                    </td>
                    <td className={td}><Pill value={visit.status} /></td>
                    <td className={td}><PriorityPill value={visit.priority} /></td>
                    <td className={td}>{visit.visited_at ? relativeLabel(new Date(visit.visited_at)) : "—"}</td>
                    <td className={`${td} text-right`}>
                      <Link href={`${basePath}/visitation/${visit.visitid || visit.id}`} className={rowAction}>
                        Open
