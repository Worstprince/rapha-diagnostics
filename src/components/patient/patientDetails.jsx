    ...
    if (text) {
        if (contentType.includes("application/json")) {
            ...
            if (text.trim().startsWith("<!DOCTYPE") || text.trim().startsWith("<html")) {
                throw new Error(
                    `The API returned an HTML page instead of JSON (${response.status}). Check the API route: ${url}`
                );
            }
    ...
    return (
        <div className="rd-panel flex flex-col overflow-hidden">
            ...
            <div className="flex flex-none items-center gap-3 border-b border-rd-hair p-4">
                ...
                <span
                    className={`${tint} grid size-10 flex-none place-items-center rounded-xl`}
                >
                    <Glyph size={18} />
                </span>
                ...
                <div className="min-w-0">
                    ...
                    <h2 className="text-base font-semibold text-rd-title">
                        {title}
                    </h2>

                    <p className="mt-0.5 truncate text-xs text-rd-muted">
                        {caption}
                    </p>
    ...
    const fetchHistory = useCallback(
        async (testId, field) => {
            ...
            try {
                ...
                const data = await fetchJson(
                    `/api/doctor/patients/${id}/trends?${params.toString()}`
                );
    ...
    const fetchChartData = useCallback(
        async (testId, field) => {
            ...
            try {
                ...
                const data = await fetchJson(
                    `/api/doctor/patients/${id}/chart?${params.toString()}`
                );
                                            ...
                                            <div className="flex min-w-0 flex-1 flex-wrap items-center justify-between gap-x-8 gap-y-4 rounded-2xl border border-rd-hair bg-rd-sunken p-4 transition duration-200 group-hover:border-rd-cyan/45 group-hover:shadow-[var(--rd-lift)]">
                                                ...
                                                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-10 gap-y-4">
                                                    ...
                                                    <div className="min-w-[8rem]">
                                                        ...
                                                        <p
                                                            className={`mt-1 text-base font-semibold ${recencyTone(
                                                                date
                                                            )}`}
                                                        >
                                                            {relativeLabel(
                                    ...
                                    ({
                                        ...
                                        >
                                            ...
                                            <div className="flex items-start justify-between gap-3">
                                                ...
                                                <p className="text-sm font-medium text-rd-label">
                                                    {
                                                        label
                                                    }
                                                </p>
